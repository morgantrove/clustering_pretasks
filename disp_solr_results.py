...
def main(args):
    ...
    arg = args[0]
    content_index = arg.find("content:")
    if not -1 == content_index:
        if -1 == arg[content_index+8:].find("6502259500"):
            spl = arg.split(":",1)
            spl.insert(1, "6502259500")
            arg.join(spl)
        

    # Argument 'arg' taken as the channel ID number.
    r_str = "http://builder-solr-slave.dev.trove.com:7979/solr/select/?q=" + arg + \
                        "&qt=troveapihandler&recency=1&rows=100&fl=channel" 
